...
rpc_light::array_t return_array(rpc_light::array_t array)
{
    ...
    for (auto &e : array)
        double_array.push_back(e.get_alt<double>() * 2);
    return double_array;
}
...
int main(int argc, char **argv)
{
    ...
    auto batch_request = client.create_batch(
        ...
        client.create_request("convert", 5, {true}),
    ...
    if (batch_server.is_batch())
    {
        ...
        {
            if (e.has_error())
                std::cout << "id: " << e.get_id().get_alt<int>() << ": server error: " << e.get_message() << " " << e.get_data().get_alt<std::string>() << std::endl;
        }
    }
    ...
    if (batch_client.is_batch())
    {
        ...
        {
            if (e.has_error())
                std::cout << "id: " << e.get_id().get_alt<int>() << ": client error: " << e.get_message() << " " << e.get_data().get_alt<std::string>() << std::endl;
        }
    }
}